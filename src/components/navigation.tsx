...
export default function Navigation() {
 ...
 return (
  <nav className="fixed top-5 sm:top-10 right-5 sm:right-10 z-[999] transition-colors items-start flex gap-5">
         ...
         {section.title === 'Projects' &&
          sectionIndex === Sections.Projects && (
           <div className="flex ml-2 mt-1 space-x-1 absolute">
            {[...Array(PROJECT_SUBSECTIONS)].map((_, subIndex) => (
             <motion.button
     ...
     >
      {sectionNavigationOptions.map((section, i) => (
       ...
       >
        ...
        {/* Mobile Subsection Indicators - Redesigned to match reference image */}
        <AnimatePresence>
         {/* Projects Section Mobile Indicators */}
         {section.title === 'Projects' &&
          sectionIndex === Sections.Projects && (
           <div className="flex justify-center items-center gap-3 my-1">
            {[...Array(PROJECT_SUBSECTIONS)].map((_, subIndex) => (
             <motion.button
              onClick={() => {
               setSubsectionIndex(subIndex);
               setIsHidden(true);
              }}
              key={subIndex}
              className={`rounded-full w-4 h-4 transition-all duration-200 
               ${
                subsectionIndex === subIndex
                 ? 'bg-theme-primary opacity-100'
                 : 'bg-theme-primary opacity-50'
               }`}
             />
            ))}
           </div>
   ...
   {/* Original Hamburger Menu Button */}
   <div
    onClick={toggle}
